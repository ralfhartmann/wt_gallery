...
class tx_wtgallery_pi1 extends tslib_pibase {
	...
	if ($this->conf['main.']['path.']['field']) {
		$lookup_uid = $GLOBALS['TSFE']->id;
		$lookup_fieldName = $this->conf['main.']['path.']['field'];

		// get current DS Data record
		$flexXML = mysql_query( "SELECT tx_templavoila_flex, pid FROM pages WHERE uid = $lookup_uid" );
		if (mysql_num_rows( $flexXML ) >= 1) {
			// output the database info
			// $this->content .= "<blockquote>".mysql_result($flexXML,0,0)."</blockquote><br />";
			...
			if (is_array( $XML ))
			{
				...
				$this->conf['main.']['path'] = $currFieldValue;
			}
		}
		}
		if ($this->conf['main.']['path'])
		{ // if picture path exists
			if (substr( $this->conf['main.']['path'], - 1, 1 ) != '/')
				$this->conf['main.']['path'] .= '/'; // add slash at the end if this is missing
			if (substr( $this->conf['main.']['path'], 0, 1 ) == '/')
				$this->conf['main.']['path'] = substr( $this->conf['main.']['path'], 1 ); // remove first slash if exits
			if (! t3lib_div::validPathStr( $this->conf['main.']['path'] ))
